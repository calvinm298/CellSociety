...
/**
 * @author August Ning Specific parser used for Wa-Tor World
 *
 */
public class WatorParser extends XMLParser {
	...
	public void parseGameConstants() {
		try {
			this.maxChronCount = (Integer
					.parseInt(this.getDoc().getElementsByTagName("max_chron_count").item(0).getTextContent()));
			this.sharkStartingEnergy = (Integer
					.parseInt(this.getDoc().getElementsByTagName("shark_starting_energy").item(0).getTextContent()));
			this.sharkMaxEnergy = Integer
					.parseInt(this.getDoc().getElementsByTagName("shark_max_energy").item(0).getTextContent());
